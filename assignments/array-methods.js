...
let fullNames = [];
let names = runners.forEach(function(fullName) {
...
let bigNames = runners.map(function(capitlize) {
	firstNamesAllCaps.push(capitlize.first_name.toUpperCase());
});
...
let runnersLargeSizeShirt = runners.filter(
	shirtSize => shirtSize.shirt_size === "L",
);
...
let ticketPriceTotal = runners.reduce(function(accumulator, donations) {
	return accumulator + donations.donation;
}, 0);
...
let company$ = [];
let company = runners.forEach(function(companies) {
	company$.push(`${companies.company_name} ${companies.donation}`);
});
console.log(company$);
//@ Problem 2
//@ map emails
let personalEmail = [];
let emails = runners.map(function(email) {
	personalEmail.push(` ${email.first_name} ${email.email} `);
});
...
let distanceRun = runnersDistance.reduce(function(accumulator, reach) {
	return accumulator + reach.distance;
}, 0);
