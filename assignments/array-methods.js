...
let fullNames = [];
let names = runners.forEach(function(fullName){fullNames.push (`${fullName.first_name} ${fullName.last_name}`)});
...
let bigNames = runners.map(function(capitlize){firstNamesAllCaps.push (capitlize.first_name.toUpperCase())})
console.log(firstNamesAllCaps);

//* ==== Challenge 3: Use .filter() ====
//* The large shirts won't be available for the event due to an ordering issue. We need a filtered version of the runners array, containing only those runners with large sized shirts so they can choose a different size. This will be an array of objects.
let runnersLargeSizeShirt = runners.filter(function(shirtSize) { return shirtSize.shirt_size === "L"});
console.log(runnersLargeSizeShirt);
  
//* ==== Challenge 4: Use .reduce() ====
//* The donations need to be tallied up and reported for tax purposes. Add up all the donations and save the total into a ticketPriceTotal variable.
let ticketPriceTotal = runners.reduce (function(accumulator, donations){return accumulator + donations.donation },0);
...
//* Problem 1
//*Which companies provided donations
let company$ = [];
let company = runners.forEach(function(companies){company$.push (`${companies.company_name} ${companies.donation}`)});
console.log(company$)
//* Problem 2
//* map emails
let personalEmail = [];
let emails = runners.map(function (email){personalEmail.push (` ${email.first_name} ${email.email} `)});
...
let distanceRun = runnersDistance.reduce (function(accumulator, reach){return accumulator + reach.distance},0);
